...
const CourseModuleCard = ({number, title, reading, video, instructor, percent, isActive, link}) => {
  return (
      <div className={` md:w-[720px] md:h-[80px] flex flex-col md:flex-row justify-center items-center rounded-[8px] gap-x-5 md:gap-x-[56px] px-[20px] md:px-[24px] py-[15px] md:py-0`} style={{backgroundColor: isActive ? 'white' : '#e0e0e0'}}>
          <div className="w-[20p md:w-[64px] h-[20px] md:h-[64px] rounded-[8px] hidden md:flex items-center justify-center bg-slate-600 ">
              <p className='font-space font-[700] text-[16px] md:text-[32px] leading-[12px] md:leading-[41px]'>{ number}</p>
          </div>
          ...
          <div className="flex items-center justify-center">
              {isActive ? 
                <Link href={`/auth/modules/${link}`}> <p className='font-space font-[400] text-[30px] leading-[20px]'>...</p> </Link> :
