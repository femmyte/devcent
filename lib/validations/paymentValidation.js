...
export async function validatePaymentInfo(data) {
...
export async function validatePayment(data) {
  const schema = yup.object().shape({
    ...
    //   .label("Created at"),
    course: yup.string().required("Course is required").label("Course"),
    tx_ref: yup.string().required("Tx reference is required").label("Tx ref"),
